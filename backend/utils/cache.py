...
class Cache:
    ...
    def clear_by_prefix(self, prefix: str) -> None:
        """清除指定前缀的缓存（Master: SCAN + DELETE_MANY，避免 KEYS 阻塞）"""
        pattern = f"{self.prefix}:{prefix}*" if self.prefix else f"{prefix}*"
        try:
            from django_redis import get_redis_connection
            client = get_redis_connection('rw_default' if 'rw_default' in getattr(settings, 'CACHES', {}) else 'default')
    ...
    def get_or_set_with_lock(
        ...
        # 2. 缓存 miss → 尝试获取互斥锁（Master）
        lock_key = f"{cache_key}:lock"

        try:
            from django_redis import get_redis_connection
            # 锁操作必须走 Master
            redis_client = get_redis_connection('rw_default' if 'rw_default' in getattr(settings, 'CACHES', {}) else 'default')
        except (ImportError, Exception):
            return fetch_func()
