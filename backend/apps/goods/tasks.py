...
class RecalcSalesRankingTask(CeleryTask):
    ...
    def exec(self, task_self):
        ...
        # 清除旧排行
        redis.delete('goods:ranking:sales')

        # 按销量重算 ZSET (分页遍历，避免全量加载)
        for sku in SKU.objects.filter(shelf_status='on', stock__gt=0).values('id', 'sales'):
            redis.zadd('goods:ranking:sales', {sku['id']: sku['sales']})

        redis.expire('goods:ranking:sales', 3600 * 2)
        logger.info('Sales ranking recalculated')
...
class DatabaseBackupTask(CeleryTask):
    ...
    def run(self):
        ...
        cmd = (
            f'mysqldump -h {db_host} -u root -p{db_password} '
            f'--single-transaction --routines --triggers --events '
            f'{db_name} | gzip > {filepath}'
        )
