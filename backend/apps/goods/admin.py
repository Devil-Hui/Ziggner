...
# ── Read-only fallback: daily ops use /api/goods/* admin DRF endpoints ──
def _readonly(admin_cls):
    admin_cls.actions = None
    original_get_readonly = admin_cls.get_readonly_fields

    def get_readonly_fields(self, request, obj=None):
        # 超级管理员可完整编辑；普通管理员全字段只读（日常运维走 /api/goods/* DRF）
        if request.user.is_superuser:
            return original_get_readonly(self, request, obj)
    ...
    admin_cls.get_readonly_fields = get_readonly_fields
    admin_cls.has_add_permission = has_add_permission
    admin_cls.has_change_permission = has_change_permission
    admin_cls.has_delete_permission = has_delete_permission
    return admin_cls


for _admin in list(admin.site._registry.values()):
    if _admin.__module__ == __name__:
        _readonly(_admin.__class__)
