...
from ..admin_permissions import IsStaffOrAbove, IsGroupLeader, IsSuperUser, can_operate_spu, is_superuser, get_group_managed_category_ids
...
class SPUAdminListView(BaseApiView):
    ...
    def get(self, request):
        queryset = SPU.objects.filter(deleted_at__isnull=True).select_related('brand', 'category', 'submitted_by', 'reviewed_by')
...
class SPUAdminAuditView(BaseApiView):
    ...
    def post(self, request, spu_id):
        ...
        if not can_audit_spu(spu, request.user):
            return Response({'detail': Messages.ADMIN_SPU_NOT_IN_GROUP}, status=status.HTTP_403_FORBIDDEN)
