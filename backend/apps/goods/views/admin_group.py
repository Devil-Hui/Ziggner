...
from apps.users.account import resolve_user_by_account_no
...
def _resolve_group(group_ref) -> tuple:
    """按数字 id 或 slug 解析管理组，兼容前端两种寻址方式。

    返回 (group, None) 或 (None, error_message)。前端（Cloudflare 构建）以 slug
    寻址分组（如默认收容组 ``pending``），旧调用方可能仍传数字 id，两者都支持。
    """
    try:
        gid = int(group_ref)
        group = AdminGroup.objects.filter(id=gid, is_active=True).first()
    except (ValueError, TypeError):
        group = AdminGroup.objects.filter(slug=group_ref, is_active=True).first()
    if group is None:
        return None, Messages.ADMIN_GROUP_NOT_FOUND
    return group, None
...
class AdminGroupMembersView(BaseApiView):
    ...
    def get(self, request, group_ref):
        ...
        for m in members:
            ...
            items.append({
                'account_no': account_no or '', 'username': m.user.username,
    ...
    def post(self, request, group_ref):
        group, err = _resolve_group(group_ref)
        if group is None:
            return Response({'detail': err}, status=status.HTTP_404_NOT_FOUND)
        ...
        # 前端以 account_no 指认成员（不暴露内部 id、不以 PII 查询）
        account_no = (request.data.get('account_no') or '').strip()
        if not account_no:
            return Response({'detail': Messages.ADMIN_GROUP_MEMBER_INVALID}, status=status.HTTP_400_BAD_REQUEST)
        try:
            target_user = resolve_user_by_account_no(account_no)
        except Exception:
            return Response({'detail': Messages.ADMIN_GROUP_MEMBER_NOT_FOUND}, status=status.HTTP_404_NOT_FOUND)

        if AdminGroupMember.objects.filter(group_id=group.id, user=target_user).exists():
            return Response({'detail': Messages.ADMIN_GROUP_MEMBER_EXISTS}, status=status.HTTP_409_CONFLICT)
        member = AdminGroupMember.objects.create(group_id=group.id, user=target_user, role=role)
        create_audit_log(request.user, 'admin_group.member_add', 'admin_group_member', member.id,
                         changes={'group_id': group.id, 'account_no': account_no, 'role': role},
                         ip_address=request.META.get('REMOTE_ADDR'))
        return Response({'id': member.id, 'account_no': account_no, 'role': member.role}, status=status.HTTP_201_CREATED)

    @extend_schema(
        request=None,
        responses={200: OpenApiResponse(description='Member removed')}
    )
    def delete(self, request, group_ref, user_ref):
        group, err = _resolve_group(group_ref)
        if group is None:
            return Response({'detail': err}, status=status.HTTP_404_NOT_FOUND)
        ...
        if not is_super:
            ...
            if group.id not in leader_groups:
                return Response({'detail': Messages.PERMISSION_DENIED}, status=status.HTTP_403_FORBIDDEN)
            target = AdminGroupMember.objects.filter(group_id=group.id, user__profile__account_no=user_ref).first()
            if target and target.role == AdminGroupMember.Role.LEADER:
                return Response({'detail': Messages.PERMISSION_DENIED}, status=status.HTTP_403_FORBIDDEN)
        deleted, _ = AdminGroupMember.objects.filter(group_id=group.id, user__profile__account_no=user_ref).delete()
        create_audit_log(request.user, 'admin_group.member_remove', 'admin_group_member', 0,
                         changes={'group_id': group.id, 'account_no': user_ref},
                         ip_address=request.META.get('REMOTE_ADDR'))
...
class AdminGroupUpdateView(BaseApiView):
    ...
    def put(self, request, group_ref):
        group, err = _resolve_group(group_ref)
        if group is None:
            return Response({'detail': err}, status=status.HTTP_404_NOT_FOUND)
...
class AdminGroupDeleteView(BaseApiView):
    ...
    def delete(self, request, group_ref):
        group, err = _resolve_group(group_ref)
        if group is None:
            return Response({'detail': err}, status=status.HTTP_404_NOT_FOUND)
