...
from ..admin_permissions import IsSuperUser, IsStaffOrAbove, can_operate_spu
...
# 所有 Tag CRUD 仅限超管（IsSuperUser）操作，无需组隔离。
...
class SPUTagSetView(BaseApiView):
    ...
    def post(self, request):
        ...
        for tid in tag_ids:
            SPUTagRelation.objects.create(spu_id=spu_id, tag_id=tid)
