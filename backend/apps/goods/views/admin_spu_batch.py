...
from ..admin_permissions import IsStaffOrAbove, IsGroupLeader, can_operate_spu
...
class SPUAdminBatchView(BaseApiView):
    ...
    def post(self, request):
        ...
        spus = SPU.objects.filter(id__in=spu_ids, deleted_at__isnull=True)
        if not spus.exists():
        ...
        if action == 'batch_audit':
            if not request.user.is_superuser and not request.user.admin_group_memberships.filter(role='leader', status=1).exists():
