...
def _review_to_dict(r: Review) -> dict:
    replies = getattr(r, 'active_replies', None)
    if replies is None:
        replies = []
    return {
        'id': r.id,
        ...
        'replies': [_review_to_dict(reply) for reply in replies],
        'user': {
...
class ReviewService:
    ...
    @staticmethod
    def _invalidate(spu_id, *user_ids):
        _cache.set(f'spu:{spu_id}:list-version', uuid4().hex, 3600)
        _cache.delete(f'spu:{spu_id}:stats')
        for user_id in set(user_ids):
            if user_id is not None:
                _cache.set(f'user:{user_id}:list-version', uuid4().hex, 3600)
    ...
    def create(user, spu_id, rating, content='', images=None, order_item_id=None,
        ...
        ReviewService._invalidate(spu_id, user.id)
        return review

    @staticmethod
    def list_by_spu(spu_id, page=1, per_page=20):
        version = _cache.get(f'spu:{spu_id}:list-version', 'initial')
        cache_key = f'spu:{spu_id}:list:{version}:{page}:{per_page}'
        cached = _cache.get_json(cache_key)
        if cached is not None:
            return cached['results'], cached['total']

        active_replies = Review.objects.filter(is_active=True).select_related('user')
        qs = Review.objects.filter(
            spu_id=spu_id,
            is_active=True,
            parent__isnull=True,
        ).select_related('user').prefetch_related(
            Prefetch('replies', queryset=active_replies, to_attr='active_replies'),
        )
        total = qs.count()
    ...
    def list_by_user(user, page=1, per_page=20):
        version = _cache.get(f'user:{user.id}:list-version', 'initial')
        key = f'user:{user.id}:{version}:{page}:{per_page}'
        cached = _cache.get_json(key)
        if cached is not None:
            return cached['results'], cached['total']
        active_replies = Review.objects.filter(is_active=True).select_related('user')
        qs = Review.objects.filter(user=user).select_related('spu', 'user').prefetch_related(
            Prefetch('replies', queryset=active_replies, to_attr='active_replies'),
        )
        total = qs.count()
        results = [_review_to_dict(r) for r in qs[(page - 1) * per_page:page * per_page]]
    ...
    def invalidate_user_cache(user):
        _cache.set(f'user:{user.id}:list-version', uuid4().hex, 3600)
    ...
    def update(user, review_id, **kwargs):
        ...
        for k, v in kwargs.items():
            if v is not None and k in allowed:
                setattr(review, k, v)
        review.save()
        ReviewService._invalidate(review.spu_id, user.id)
        return review
    ...
    def create_reply(user, parent_id, rating, content='', images=None, is_anonymous=False):
        ...
        ReviewService._invalidate(parent.spu_id, user.id, parent.user_id)
        return reply
