...
class CustomerServiceConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.user = user
        self.is_admin = getattr(user, 'is_staff', False)
    ...
    async def _check_rate_limit(self) -> bool:
        """
        滑动窗口计数器检查
        Redis key: cs:rate_limit:user:{user_id}
        每个请求用排序集合记录时间戳，60s 窗口内超过 30 条则拒绝
        """
        try:
            redis_client = await database_sync_to_async(get_redis_client)()
            ...
            # 1. 清理过期条目
            await database_sync_to_async(redis_client.zremrangebyscore)(key, 0, window_start)

            # 2. 计数
            count = await database_sync_to_async(redis_client.zcard)(key)

            if count >= RATE_LIMIT_MAX:
                return False

            # 3. 添加当前请求
            await database_sync_to_async(redis_client.zadd)(key, {str(now): now})
            await database_sync_to_async(redis_client.expire)(key, RATE_LIMIT_WINDOW + 10)
    ...
    def _can_access_conversation(self) -> bool:
        ...
        if self.is_admin:
            ...
            if self.user.is_superuser:
                return Conversation.objects.filter(id=self.conv_id).exists()
            # 管理组成员: 仅本组会话
        ...
        return Conversation.objects.filter(id=self.conv_id, user=self.user).exists()
