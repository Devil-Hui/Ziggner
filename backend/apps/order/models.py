...
class Order(models.Model):
    ...
    def cancel(self, reason=''):
        if self.status not in (OrderStatus.PENDING_PAYMENT, OrderStatus.PAID):
            raise ValueError(
                ...
                f'Only Pending Payment or Paid orders can be cancelled.'
            )
        # 恢复库存
        if self.status == OrderStatus.PAID:
            self._restore_stock()
        # 恢复优惠券
        self._restore_coupons()
    ...
    def _restore_coupons(self):
        """订单取消时恢复已使用的优惠券"""
        from apps.promotion.models import UserCoupon
        used_coupons = UserCoupon.objects.filter(
            used_order_no=self.order_no,
            status='used',
        ).select_for_update()
        for uc in used_coupons:
            ...
            uc.used_order_no = ''
            uc.used_at = None
            uc.save(update_fields=['status', 'used_order_no', 'used_at'])
