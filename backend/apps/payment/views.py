...
from .serializers import (
    ...
    RefundStatusSerializer, MockPaymentScenarioSerializer,
)
...
class MockPaymentCompleteView(BaseApiView):
    ...
    @extend_schema(
        ...
        responses={200: OpenApiResponse(description='Mock payment scenario processed')},
    )
    def post(self, request, payment_no):
        serializer = MockPaymentScenarioSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        try:
            result = PaymentService.simulate_mock_payment(
                request.user,
                payment_no,
                serializer.validated_data['scenario'],
            )
        except ValueError as exc:
            if str(exc) in {'MOCK_PAYMENT_UNAVAILABLE', 'PAYMENT_NOT_FOUND'}:
                return Response({'detail': Messages.PAYMENT_NOT_FOUND}, status=404)
            raise
        return Response(result)
...
class RefundView(BaseApiView):
    ...
    def post(self, request):
        ...
        serializer = CreateRefundSerializer(data=payload)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        try:
