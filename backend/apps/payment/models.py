...
def generate_payment_no():
    return timezone.now().strftime('%Y%m%d%H%M%S') + ''.join(_SECURE_RANDOM.choices(string.digits, k=6))
...
class PaymentEvent(models.Model):
    ...
    payment = models.ForeignKey(
        PaymentLog, null=True, blank=True, on_delete=models.SET_NULL, related_name='events',
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['gateway', 'event_id'], name='uniq_payment_gateway_event'),
        ]
        indexes = [models.Index(fields=['gateway', 'event_id'])]


def generate_refund_no():
    return 'RF' + timezone.now().strftime('%Y%m%d%H%M%S') + ''.join(
        _SECURE_RANDOM.choices(string.digits, k=6)
    )


def generate_gateway_request_id():
...
class RefundLog(models.Model):
    ...
    refund_no = models.CharField(max_length=32, unique=True, default=generate_refund_no,
                                 verbose_name='退款流水号')
    idempotency_key = models.CharField(max_length=128, verbose_name='客户端幂等键')
    gateway_request_id = models.CharField(
        max_length=64, unique=True, default=generate_gateway_request_id,
        verbose_name='网关请求ID',
    )
