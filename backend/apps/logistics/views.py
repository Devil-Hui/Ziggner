...
class ShippingCostView(BaseApiView):
    ...
    def post(self, request):
        ...
        result = LogisticsService.calculate_shipping_cost(request.user, request.data)
...
class TrackShipmentView(PublicApiView):
    ...
    def get(self, request):
        tracking_no = request.query_params.get('tracking_no', '').strip()
        if not tracking_no:
            return Response({'detail': 'tracking_no is required'}, status=status.HTTP_400_BAD_REQUEST)
        try:
