...
export const publicAPI = {
  ...
  login: (username: string, password: string) =>
    post<{ access: string; refresh: string }>('/users/login/', { username, password }),
  ...
  getMyCoupons: (params?: { status?: string; page?: number; page_size?: number }) =>
    get<PageResponse<UserCoupon>>('/promotion/my/', params),

  // ── 购物车 ──
  /** 获取购物车 */
  getCart: () =>
    get<{ id: number; items: CartItem[]; total: number; count: number }>('/cart/'),

  /** 添加商品到购物车 */
  addToCart: (skuId: number, quantity: number) =>
    post<CartItem>('/cart/items/', { sku_id: skuId, quantity }),

  /** 更新购物车项数量 */
  updateCartItem: (itemId: number, quantity: number) =>
    put<CartItem>(`/cart/items/${itemId}/`, { quantity }),  // 注意：后端使用 PUT，前端保持一致
  ...
  /** 申请退款 */
  requestRefund: (orderNo: string, reason?: string, amount?: number) =>
    post('/payment/refund/', { order_no: orderNo, reason: reason || '', amount }),
