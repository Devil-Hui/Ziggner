...
import { get, post as apiPost } from '../api/request'
import { setUserAccessToken, setUserRefreshToken, getUserAccessToken, clearUserTokens } from '../api/request'
...
interface UserContextValue {
  ...
  register: (username: string, password: string, email?: string, phone?: string, verifyId?: string, verifyCode?: string) => Promise<{ success: boolean; error?: string }>
  logout: () => void
...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (username: string, password: string) => {
    try {
      ...
      const res: any = await publicAPI.login(username, password)
      const payload = res && res.data ? res.data : res
      const { access, refresh } = payload || {}
      if (!access) return { success: false, error: '登录失败：服务器未返回 token' }
      setUserAccessToken(access)
      if (refresh) setUserRefreshToken(refresh)
      await refreshUser()
      return { success: true }
    } catch (err: unknown) {
  ...
  const register = useCallback(async (username: string, password: string, email?: string, phone?: string, verifyId?: string, verifyCode?: string) => {
    try {
  ...
  const socialLogin = useCallback(async (provider: string, accessToken: string) => {
    try {
      ...
      if (!access) throw new Error('登录失败：服务器未返回 token')
      setUserAccessToken(access)
      if (refresh) setUserRefreshToken(refresh)
