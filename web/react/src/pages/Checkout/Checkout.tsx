...
export default function Checkout() {
  ...
  // ── 价格计算 ──
  const coupon = selectedCoupon ? coupons.find(c => c.code === selectedCoupon) : null
  let discount = 0
  if (coupon) {
    if (coupon.discount_type === 'fixed') {
      discount = total >= parseFloat(coupon.min_amount || '0') ? coupon.amount : 0
    } else {
      discount = Math.min(
        total * (coupon.amount / 100),
        parseFloat(coupon.max_discount || '0') || Infinity
      )
    }
  }
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const paymentResult = await paymentAPI.create({
        order_no: orderNo,
        method: paymentMethod as 'paypal' | 'stripe',
        ...
        <Container>
          ...
          <LayoutGrid>
              ...
              <Card>
                ...
                {couponsLoading ? (
                  ...
                  coupons.map(c => (
                    <CouponOption
                      key={c.code}
                      $selected={selectedCoupon === c.code}
                    ...
                    >
                      ...
                      <CouponDot $active={selectedCoupon === c.code} />
                    </CouponOption>
                  ))
                )}
                ...
                {!couponsLoading && coupons.length > 0 && (
                  ...
                  >
                    ...
                    <CouponDot $active={selectedCoupon === null} />
                  </CouponOption>
                )}
              </Card>
            ...
            <RightColumn>
              <Card>
                <CardHeader>
                  <CardTitle style={{ fontSize: 17 }}>Order Summary</CardTitle>
                </CardHeader>
                ...
                <RedButton onClick={handlePlaceOrder} disabled={loading} style={{ marginTop: 20 }}>
                  {loading ? t('store.checkout.placingOrder') : `Place Order — $${actual.toFixed(2)}`}
                </RedButton>
