/**
 * Icon 组件 — Feather Icons 风格 SVG（MIT 协议）
 * 替代 emoji 和分散的内联 SVG，统一管理 Admin 所有图标
 * 用法：<Icon name="plus" size={18} />
 *
 * 所有图标 24×24 viewBox，strokeWidth=2，默认 16px
 * 颜色继承 currentColor，可通过 color prop 覆盖
 */
...
export const Icon: React.FC<IconProps> = ({ name, size = 16, color, className, title }) => {
  const ko = koboyoIcons[name]
  if (ko) {
    const isStroke = ko.mode === 'stroke'
    return (
      <svg
      ...
      >
        ...
        <g dangerouslySetInnerHTML={{ __html: ko.inner }} />
      </svg>
    )
  }
